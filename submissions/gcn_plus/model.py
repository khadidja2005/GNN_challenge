"""
GCN+ submission for ENZYMES-Hard.
Fast CPU-only training (<5m) with mean imputation and compact GCN (<100K params).

Usage:
  python submissions/gcn_plus/model.py \
    --data_dir data/challenge \
    --output_dir submissions/gcn_plus \
    --epochs 60
"""
...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="submissions/gcn_plus", help="Output directory")
    p.add_argument("--epochs", type=int, default=60, help="Training epochs (keep small for speed)")
...
def main():
    ...
    model = GCNPlus(in_dim, args.hidden_dim, args.num_layers, dropout=args.dropout).to(device)
