"""
ENZYMES-Hard Challenge: GCN Submission

Trains a compact GCN (<100K params) and emits a predictions.csv submission file.

Usage:
    python submissions/gcn_submission.py --data_dir data/challenge --output_dir submissions/gcn
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="GCN submission")
    p.add_argument("--data_dir", type=str, default="data/challenge", help="Path to challenge data")
    p.add_argument("--output_dir", type=str, default="submissions/gcn", help="Output directory")
    p.add_argument("--epochs", type=int, default=140, help="Training epochs")
    p.add_argument("--batch_size", type=int, default=32, help="Batch size")
    p.add_argument("--hidden_dim", type=int, default=64, help="Hidden dimension")
    p.add_argument("--num_layers", type=int, default=3, help="GCN layers")
...
class GCNNet(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 64, num_layers: int = 3, num_classes: int = 6, dropout: float = 0.5):
        ...
        self.convs.append(GCNConv(in_dim, hidden_dim, cached=False))
...
def main():
    ...
    model = GCNNet(in_dim, args.hidden_dim, args.num_layers, dropout=args.dropout).to(device)
