"""
ENZYMES-Hard Challenge: GAT Submission

Lightweight Graph Attention Network (GAT) staying under 100K parameters.
Trains on challenge splits and writes a submission CSV.

Usage:
    python submissions/gat_submission.py --data_dir data/challenge --output_dir submissions/gat
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="GAT submission")
    p.add_argument("--data_dir", type=str, default="data/challenge", help="Path to challenge data")
    p.add_argument("--output_dir", type=str, default="submissions/gat", help="Output directory")
    p.add_argument("--epochs", type=int, default=120, help="Training epochs")
    p.add_argument("--batch_size", type=int, default=32, help="Batch size")
    p.add_argument("--hidden_dim", type=int, default=32, help="Hidden dim per head")
    p.add_argument("--heads", type=int, default=4, help="Number of attention heads")
    p.add_argument("--num_layers", type=int, default=3, help="GAT layers")
...
class GATNet(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 32, heads: int = 4, num_layers: int = 3, num_classes: int = 6, dropout: float = 0.5):
        super().__init__()
        ...
        self.convs.append(GATConv(in_dim, out_channels, heads=heads, dropout=dropout))
        self.bns.append(nn.BatchNorm1d(out_channels * heads))

        for _ in range(num_layers - 1):
            in_c = out_channels * heads
            self.convs.append(GATConv(in_c, out_channels, heads=heads, dropout=dropout))
            self.bns.append(nn.BatchNorm1d(out_channels * heads))
...
def main():
    ...
    model = GATNet(in_dim, args.hidden_dim, args.heads, args.num_layers, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 10 == 0:
            ...
            print(f"Epoch {epoch:03d} | loss {loss:.4f} | val F1 {val_f1:.3f} | time {elapsed:.1f}s")
